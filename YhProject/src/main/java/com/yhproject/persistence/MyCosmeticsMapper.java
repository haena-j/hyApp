...
public interface MyCosmeticsMapper {
    @Insert("INSERT INTO MY_COSMETICS(m_open_date, m_expire_date, cos_index, member_index, m_review, m_starrate, m_cosimage) VALUES(#{m_open_date}, #{m_expire_date}, #{cos_index}, #{member_index}, #{m_review}, #{m_starrate}, #{m_cosimage})")
    ...
    @Update("UPDATE MY_COSMETICS SET M_INDEX = #{m_index}, M_OPEN_DATE = #{m_open_date}, M_EXPIRE_DATE = #{m_expire_date}, COS_INDEX = #{cos_index}, MEMBER_INDEX = #{member_index}, M_REVIEW = #{m_review}, M_STARRATE = #{m_starrate}, M_COSIMAGE = #{m_cosimage} WHERE M_INDEX = #{m_index}")
...
//    void updateMemberImage(MemberVO member);
    ...
    @Select("SELECT SUM(m_starrate) FROM MY_COSMETICS WHERE cos_index = #{cos_index}")
    int getSumCosStarRates(@Param("cos_index") int cos_index);
